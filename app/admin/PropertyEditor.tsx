...
import { GripVertical, Images, ImagePlus, Plus, Star, Trash2 } from 'lucide-react';
...
export default function PropertyEditor({token}:{token:string}) {
 ...
 const [creating,setCreating]=useState(false),[newStay,setNewStay]=useState({name:'',slug:''});
 ...
 async function createStay(){
  const name=newStay.name.trim(),slug=slugify(newStay.slug||newStay.name);
  if(name.length<2||slug.length<2){setMessage('Enter a stay name and URL slug first.');return;}
  setBusy(true);setMessage('');
  try{
   const emptyContent={location:'',area:'',tag:'',heroImage:'',gallery:[],bedrooms:0,beds:0,baths:0,propertyType:'Entire home',summary:'',description:[],highlights:[],amenities:[],sleeping:[],checkIn:'',checkOut:'',parking:'',wifi:'',access:'',houseRules:[],safety:[],locationCopy:'',airbnbUrl:'',rating:'',reviewCount:0,guestFavorite:false,reviewScores:[],photoTour:[],cancellationPolicy:''};
   const r=await fetch(`${SUPABASE_URL}/rest/v1/properties?select=${propertySelect}`,{method:'POST',headers:{...supabaseHeaders(token),'Content-Type':'application/json',Prefer:'return=representation'},body:JSON.stringify({slug,name,max_guests:1,min_nights:1,max_nights:60,base_nightly_rate:null,weekend_nightly_rate:null,cleaning_fee:null,tax_rate:0,active:false,content:emptyContent})});
   const data=await r.json().catch(()=>null);if(!r.ok||!Array.isArray(data)||data.length!==1)throw Error(data?.message??'Could not create the stay. The URL slug may already be in use.');
   const row=data[0] as ManagedProperty;setRows(current=>[...current,row].sort((a,b)=>a.name.localeCompare(b.name)));select(row);setCreating(false);setNewStay({name:'',slug:''});setMessage('New stay created as a draft. Fill in the sections below, save, then turn on “Show this stay publicly” when it is ready.');
  }catch(e){setMessage(e instanceof Error?e.message:'Could not create the stay.');}finally{setBusy(false);}
 }
 ...
 async function save(e:FormEvent){e.preventDefault();if(!draft)return;setBusy(true);setMessage('');try{
  ...
  setRows(rows.map(row=>row.id===id?data[0]:row));setDraft(data[0]);setDirty(false);setMessage(data[0].active?'Saved and published.':'Saved as a private draft.');
 ...
 async function upload(file:File,sectionIndex:number){if(!draft)return;setBusy(true);setMessage('');try{
  ...
  const sections=(draft.content.photoTour??[]).map((section,index)=>index===sectionIndex?{...section,images:[...section.images,url],thumbnail:section.thumbnail||url}:section);content('photoTour',sections);setMessage('Photo uploaded. Drag it into position, then select Save changes.');
 ...
 return <form className={styles.panel} onSubmit={save}>
  <div className={styles.panelTitle}><div><h2>Your property</h2><p>Edit the details below, then save. Existing approved reservation prices stay unchanged.</p></div><button type="button" className={styles.secondaryButton} onClick={()=>setCreating(value=>!value)}><Plus size={15}/> Add new stay</button></div>
  {creating&&<div className={styles.panel} style={{margin:'14px 0',padding:'16px'}}><strong>Create a new stay</strong><p>Create the shell first, then fill out the same property sections used for Chestnut.</p><div className={styles.formRow}><label>Stay name<input value={newStay.name} onChange={e=>setNewStay({name:e.target.value,slug:slugify(e.target.value)})} placeholder="Seaside Cottage"/></label><label>URL slug<input value={newStay.slug} onChange={e=>setNewStay({...newStay,slug:slugify(e.target.value)})} placeholder="seaside-cottage"/></label></div><div style={{display:'flex',gap:10,flexWrap:'wrap'}}><button type="button" className={styles.primaryButton} disabled={busy} onClick={()=>void createStay()}>{busy?'Creating…':'Create stay'}</button><button type="button" className={styles.secondaryButton} onClick={()=>{setCreating(false);setNewStay({name:'',slug:''});}}>Cancel</button></div></div>}
  <label>Choose property<select disabled={busy} value={draft.id} onChange={e=>{if(dirty&&!window.confirm('Discard unsaved property changes?'))return;select(rows.find(r=>r.id===e.target.value)!);}}>{rows.map(r=><option key={r.id} value={r.id}>{r.name}{r.active?'':' (draft)'}</option>)}</select></label>
  <fieldset disabled={busy} className={styles.editorFields}>
   <details><summary>Property information & descriptions</summary>
    ...
    <label style={{display:'flex',flexDirection:'row',alignItems:'center',gap:10}}><input style={{width:18,height:18,margin:0}} type="checkbox" checked={draft.active} onChange={e=>{setDraft({...draft,active:e.target.checked});setDirty(true);}}/><span><strong>Show this stay publicly</strong><br/><small>When enabled, the stay appears on the Stays page and can receive booking requests.</small></span></label>
    ...
    <div className={styles.formRow}><label>Airbnb rating<input type="number" min="0" max="5" step="0.01" value={draft.content.rating??''} onChange={e=>content('rating',e.target.value)}/></label><label>Airbnb review count<input type="number" min="0" step="1" value={draft.content.reviewCount??0} onChange={e=>content('reviewCount',Number(e.target.value))}/></label></div>
    <label style={{display:'flex',flexDirection:'row',alignItems:'center',gap:10}}><input style={{width:18,height:18,margin:0}} type="checkbox" checked={Boolean(draft.content.guestFavorite)} onChange={e=>content('guestFavorite',e.target.checked)}/><span>Guest favorite</span></label>
    {(draft.content.reviewScores??[]).map((score,i)=><div className={styles.formRow} key={`${score.label}-${i}`}><label>Review category<input value={score.label} onChange={e=>content('reviewScores',draft.content.reviewScores!.map((v,j)=>j===i?{...v,label:e.target.value}:v))}/></label><label>Score<input value={score.score} onChange={e=>content('reviewScores',draft.content.reviewScores!.map((v,j)=>j===i?{...v,score:e.target.value}:v))}/></label></div>)}
    <button type="button" onClick={()=>content('reviewScores',[...(draft.content.reviewScores??[]),{label:'',score:''}])}>Add review score</button>
   </details>
   ...
   <details><summary>Amenities & sleeping arrangements</summary>
    ...
    <button type="button" onClick={()=>content('sleeping',[...(draft.content.sleeping??[]),{room:'New bedroom',bed:'',note:''}])}>Add sleeping arrangement</button>
   </details>
   <details><summary>Property photos</summary>
    ...
    {(draft.content.photoTour??[]).map((section,i)=><section className={styles.photoSection} key={section.id}>
      ...
      {section.images.length===0&&<button type="button" className={styles.dangerButton} onClick={()=>content('photoTour',draft.content.photoTour!.filter((_,index)=>index!==i))}>Remove empty photo section</button>}
    </section>)}
   </details>
  </fieldset>
  <div className={styles.editorSave}><button className={styles.primaryButton} disabled={busy||!dirty}>{busy?'Working…':'Save changes'}</button>{draft.active&&<a href={`/properties/${draft.slug}`} target="_blank" rel="noreferrer">View public listing ↗</a>}<span role="status">{message|| (dirty?'Unsaved changes':'')}</span></div>
 </form>;
}
