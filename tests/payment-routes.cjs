...
(async()=>{
 process.env.NEXT_PUBLIC_SITE_URL='https://www.pacificstayproperties.com';process.env.STRIPE_WEBHOOK_SECRET='whsec_fixture';
 ...
 const request=(body,sig)=>new Request('https://www.pacificstayproperties.com/api/payments/webhook',{method:'POST',body,headers:{'stripe-signature':sig}});
 ...
 const pay=(origin,body)=>new Request('https://www.pacificstayproperties.com/api/payments/checkout',{method:'POST',headers:{origin,'Content-Type':'application/json'},body:JSON.stringify(body)});
 assert.equal((await checkout.POST(pay('https://evil.example',{reservationId:id}))).status,403);
 assert.equal((await checkout.POST(pay('https://www.pacificstayproperties.com',{reservationId:'invalid'}))).status,400);
 const result=await checkout.POST(pay('https://www.pacificstayproperties.com',{reservationId:id,amount:1,property:'tampered'}));assert.equal(result.status,200);
 ...
 ready=false;assert.equal((await checkout.POST(pay('https://www.pacificstayproperties.com',{reservationId:id}))).status,503);
